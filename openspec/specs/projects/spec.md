...
The system MUST enforce that `projects.workspace_id` equals the `workspace_id` of the referenced client. Any attempt to create or modify a project so that this invariant would be violated MUST be rejected, and the database MUST enforce via foreign keys that `client_id` remains valid.

#### Scenario: Inconsistent pairing rejected
...
- **AND** no project row is created or modified
